require( "ui.uieditor.menus.StartMenu.T4StartMenu_Options" )
require( "ui.uieditor.widgets.Lobby.Common.T4ButtonListItem" )
...
DataSources.StartMenuGameOptions = ListHelper_SetupDataSource( "StartMenuGameOptions", function ( controller )
	...
	if CoD.isZombie then
		table.insert( options, {
			models = {
				displayText = "MENU_ZOMBIE_RESUMEGAME_CAPS",
		...
		table.insert( options, {
			models = {
				...
				action = function ( self, element, controller, actionParam, menu )
					NavigateToMenu( menu, "T4StartMenu_Options", true, controller )
				end
			}
		} )
...
CoD.T4StartMenu_GameOptions_ZM = InheritFrom( LUI.UIElement )
CoD.T4StartMenu_GameOptions_ZM.new = function ( menu, controller )
	...
	self:setClass( CoD.T4StartMenu_GameOptions_ZM )
	self.id = "T4StartMenu_GameOptions_ZM"
	...
	self.buttonList = LUI.UIList.new( menu, controller, -1, 0, nil, true, false, 0, 0, false, false )
	self.buttonList:makeFocusable()
	self.buttonList:setLeftRight( true, false, 901, 0 )
	self.buttonList:setTopBottom( true, false, 118, 0 )
	self.buttonList:setWidgetType( CoD.T4ButtonListItem )
