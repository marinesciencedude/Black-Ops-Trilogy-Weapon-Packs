...
DataSources.ZMGameModeModes = DataSourceHelpers.ListSetup( "ZMGameModeModes", function ( f4_arg0 )
	local f4_local0 = {}
	...
	table.insert( f4_local0, {
		models = {
			text = Engine.Localize( Engine.StructTableLookupString( CoDShared.gametypesStructTable, "name", "zclassic", "name_ref" ) ),
			buttonText = Engine.Localize( Engine.StructTableLookupString( CoDShared.gametypesStructTable, "name", "zclassic", "name_ref" ) ),
			image = Engine.StructTableLookupString( CoDShared.gametypesStructTable, "name", "zclassic", "image" ),
			description = Engine.Localize( Engine.StructTableLookupString( CoDShared.gametypesStructTable, "name", "zclassic", "description" ) )
		},
		properties = {
			gametype = "zclassic",
			selectIndex = "zclassic" == f4_local2
		}
	} )
	return f4_local0
end, true )
...
LUI.createMenu.ZMGameMode = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "ZMGameMode" )
	if PreLoadFunc then
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "ZMGameMode.buttonPrompts" )
	...
	gameModeList:setDataSource( "ZMGameModeModes" )
	...
	gamesList:linkToElementModel( gamesList, "isOfficial", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f23_local0, self, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	gamesList:linkToElementModel( gamesList, "isCommunityOption", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f24_local0, self, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	...
	self:AddButtonCallbackFunction( gamesList, controller, Enum.LUIButton.LUI_KEY_START, "O", function ( element, menu, controller, model )
		if not IsSelfModelValueTrue( element, controller, "isOfficial" ) and not IsSelfModelValueTrue( element, controller, "isCommunityOption" ) then
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "ZMGameMode.buttonPrompts" ) )
	end )
