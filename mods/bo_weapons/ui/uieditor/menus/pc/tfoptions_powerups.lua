...
DataSources.TFOptionsPowerupsP1 = DataSourceHelpers.ListSetup("PC.TFOptionsPowerupsP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Powerup Spawn Amount", description = "Choose how many powerups spawn per round.", profileVarName = "more_powerups", datasource = "TFOptionsPowerups", widgetType = "dropdown"}, properties = CoD.TFPCUtil.OptionsGenericDropdownProperties})
	table.insert(f26_local0, {models = {label = "Improved Nuke", description = "Nukes now give a set amount of points per zombie killed (Minimum of 400).", profileVarName = "better_nuke", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Improved Nuke Points", description = "Choose how many points per zombie you get.", profileVarName = "better_nuke_points", lowValue = 0, highValue = 200, useMultipleOf10Display = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "BO4 Max Ammo", description = "Max Ammos refill your clip as well.", profileVarName = "bo4_max_ammo", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	...
	table.insert(f26_local0, {models = {label = "Enable Free Perk Powerup", description = "Enabling lets the free perk powerup drop.", profileVarName = "perk_powerup", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Enable Zombie Cash Powerup", description = "Adds a zombie cash powerup that can give (or takeaway) points.", profileVarName = "zcash_powerup", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Enable Packapunch Powerup", description = "Adds a packapunch powerup that temporarily upgrades the weapon you're currently holding.", profileVarName = "packapunch_powerup", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Enable Bottomless Clip Powerup", description = "Adds a bottomless clip powerup that gives you temporary infinite ammo.", profileVarName = "bottomless_clip_powerup", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Enable Zombie Blood Powerup", description = "Adds a zombie blood powerup which makes zombies ignore the player temporarily", profileVarName = "zblood_powerup", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	return f26_local0
end, true)
...
DataSources.TFOptionPowerupsCategories = DataSourceHelpers.ListSetup("PC.TFOptionPowerupsCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "POWERUP OPTIONS", tabWidget = "CoD.TFOptions_Powerups_P1"}})
...
LUI.createMenu.TFOptionsPowerupsPage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsPowerupsPage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsPowerupsPage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionPowerupsCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsPowerupsPage.buttonPrompts"))
	end)
	if f0_local3 then
