...
function specific_powerup_drop( powerup_name, drop_spot, powerup_team, powerup_location, pickup_delay, powerup_player, b_stay_forever , is_monk_cycling = false, drop_not_notify = false)
{
	...
	{
		powerup powerup_setup( powerup_name,powerup_team, powerup_location, powerup_player , 1 ,is_monk_cycling);
...
function powerup_setup( powerup_override,powerup_team, powerup_location, powerup_player, shouldplaysound = true, is_monk_cycling = false )
{
	powerup = undefined;
	...
	// eMoX EDIT : 18/04/2020 - Code updated 14/12/2025
	if(GetGametypeSetting(mutator_delayed_powerup) == BOOLMUTATOR_OFFON_ON)
	{
	 	if(!is_monk_cycling)
	 	{
			/*if(struct.only_affects_grabber == true)
			{
				...
				powerup_pre_drop_fx = POWERUP_PRE_DROP_FX;
				powerup_pre_drop_pop_fx = POWERUP_PRE_DROP_POP_FX;
			}
